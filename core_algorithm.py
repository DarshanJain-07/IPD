...
def fvp(frame, patch_size, K):
    ...
    A frame comes in. It is down sampled, a similarity matrix is created and weight masks for the image, clustering the
...
def cdf_sb_pos(C, order, B):
    ...
    POS: Plain orthogonal to skin (core rPPG method

    :param C: signal attained after averaging a patch of pixels for each frame.
              its shape is [frame, weight_mask_stat, color_channel]
    :param order: list of channel order based on pulsate amplitude strength (decreasing order)
    ...
    # Compute FFT a long time axis -> 0
